...
export CIRCLE_REPOSITORY_URL='https://github.com/feedyard/orb-common-pipeline-tasks'
export CIRCLE_PROJECT_USERNAME='feedyard'
export CIRCLE_PROJECT_REPONAME='orb-common-pipeline-tasks'
export CIRCLE_BUILD_NUM=34

https://circleci.com/api/v1.1/project/github/$CIRCLE_PROJECT_USERNAME/$CIRCLE_PROJECT_REPONAME/33?circle-token=$CIRCLE_TOKEN
...
#  while read line
#  do
#    if [[ $(curl --user $<<parameters.circle-token>>: $line | grep "\"vcs_revision\" : \"$CIRCLE_SHA1\"") ]]; then
#      continue
#    else
#      BRANCH_IS_NEW=false
#      break
...
#  # clean up
#  if [[ <<parameters.debug>> == false ]]; then
